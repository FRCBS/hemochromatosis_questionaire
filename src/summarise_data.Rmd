...
- yhdistelmä kuva vastauksista (A (grown), b (agree), c (diag), d (tiesti))
- violin mediaani viivalla ja harmaa tausta pois
...
#https://diagnostiikka.hus.fi/tutkimus?id=1024
    fALAT_HUScut =case_when( 
      is.na(Kys20fALAT) ~ NA,
      Kys20fALAT < 35  & Kys2Gender == "Female" ~ "below",
      Kys20fALAT < 50  & Kys2Gender == "Male"  ~ "below",
...
longlab <- pivot_longer(
  ...
  mutate(
    Measurement = fct_recode(
      Measurement,
      'Ferrin ug/L' = "Kys20bFerrit", 
       'Transferrin Saturation (%)'  = "Kys20cTrfesat", 
     'Haemoglobin (g/L)'=  "Kys20dHemoglobin", 
      'eMCV (fl)' = "Kys20eMCV", 
      'Alanine aminotransferase (U/L)' ="Kys20fALAT"
    )
  ) %>% 
  rename(Sex = Kys2Gender)

p <- ggplot(longlab)
p <- p + geom_violin(aes(y=Value,x=Sex))
#p <- p  + geom_boxplot(aes(y=Value,x=Sex),alpha=0.5)
p <- p + facet_wrap(.~Measurement,scales = "free")
...
# C kuinka moni tiesi
tmp <- tibble(x=c("Contacted","Responded","Knew\nalready"),y=c(82,63,6))

plotc <- ggplot(tmp)
plotc <- plotc + geom_col(aes(x=x,y=y))
plotc <- plotc + ylab("") + xlab("Questionaire contact")
plotc <- plotc + xlim(
  "Contacted","Responded","Knew\nalready")
plotc <- plotc + theme(plot.margin = unit(c(1,2,1,2),"cm"))
plotc <- plotc + theme_bw() +  theme(axis.text.x=element_text(angle=45, hjust=1))
...
plotd <- plotd + geom_bar(aes(x=Kys22Diagnosis))
plotd <- plotd + xlab("Did you get a clinical diagnosis\nfor hemochromatosis (ICD10: E83.1)?") + ylab("Count")
plotd <- plotd + theme(plot.margin = unit(c(1,2,1,2),"cm")) + xlim(
  "Yes","No","Don't know","No response")
plotd <- plotd + theme_bw()+  theme(axis.text.x=element_text(angle=45, hjust=1))
