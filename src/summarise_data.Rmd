...
```{r} 
items = data %>% 
  select(Kys23aTiredness, Kys23bJoint)
  

#colnames(items) <- c("It was useful to receive genetic information","Receiving genetic information did not make me worry")

#nlevels = rev(c("Totally agree","Partly agree","Neutral","Partly disagree","Totally disagree"))
...
p <- ggplot(data)
p <- p +  geom_histogram(aes(x=Kys20bFerrit),binwidth = 5)
p <- p + facet_wrap(. ~ Kys2Gender)#mihin kohtaan ja miten sukupuoli
p <- p + xlab("Age in years")
p
