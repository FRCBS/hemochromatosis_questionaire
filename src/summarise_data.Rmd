...
labsumsum <- labsum %>%   group_by(Kys2Gender) %>% 
  summarise(
    n_hb = sum(!is.na(Kys20dHemoglobin)),
    ...
    n_tfrsat = sum(!is.na(Kys20cTrfesat)),
  ...
  mutate(
    Statistic = factor(Statistic, labels =c("Above","Below","Median",'N',"MAD")),
    Statistic = fct_relevel(Statistic,c('N',"Above","Below","Median","MAD"))
  ) %>% arrange(Statistic)
labsumsum
...
#p <- p + geom_text(data=refvals %>% filter(Limit == "Upper") ,aes(y=Value,x=Sex,label=n),hjust=-0.6,vjust=-0.9,size=3,color="red")
...
colnames(items) <- c(
  "How has receiving genetic information affected your trust in The Blood Service? n = 38",
  "How has receiving genetic information affected your willingness to belong to The Blood Service Biobank? n = 38"
) 
...
colnames(items) <- c(
  "It was useful to receive genetic information. n = 38",
  "Receiving genetic information did not worry me. n = 38",
  "The genome data in biobank should be used more widely to promote health. n = 38",
  "I believe I can make an impact to my predisposition to haemochromatosis with my lifestyle. n = 36",
  "Receiving genetic information motivates me to take care of my health better. n = 37"
...
plotc <- plotc + ylab("Count") + xlab("Initial contact")
plotc <- plotc + xlim(
  "Contacted","Responded","Was\naware")
plotc <- plotc + theme(plot.margin = unit(c(1,2,1,2),"cm"))
plotc <- plotc + theme_classic() +  theme(
...
plotd <- plotd + xlab("Did you recieve clinical\ndiagnosis for\n haemochromatosis\n(ICD10: E83.1)?") + ylab("Count")
plotd <- plotd + theme(plot.margin = unit(c(1,2,1,2),"cm")) + xlim(
  "Yes","No","Don't know","No response")
plotd <- plotd + theme_classic() +  theme(
  axis.text.x=element_text(angle=45, hjust=1),
...
patch <-  ((plot_spacer()  | plotc |  plotd ) /  plota /  plotb  ) +
 plot_layout(widths = c(1, 5,5), heights = c(1,1.3,2)) + plot_annotation(tag_levels = 'A')
patch
```
...
ggsave(plotc, 
       filename = here("results/pub_plotA.png"),
       width = 80, 
       height=80,
       units="mm"
         )
...
ggsave(plotc, 
       filename = here("results/pub_plotA.pdf"),
       width = 80, 
       height=80,
       units="mm"
         )
...
ggsave(plotd, 
       filename = here("results/pub_plotB.png"),
       width = 80, 
       height=80,
       units="mm"
         )
...
ggsave(plotd, 
       filename = here("results/pub_plotB.pdf"),
       width = 80, 
       height=80,
       units="mm"
         )
...
ggsave(plota, 
       filename = here("results/pub_plotC.png"),
       width = 180, 
       height=80,
       units="mm"
         )
...
ggsave(plota, 
       filename = here("results/pub_plotC.pdf"),
       width = 180, 
       height=80,
       units="mm"
         )
...
ggsave(plotb, 
       filename = here("results/pub_plotD.png"),
       width = 180, 
       height=120,
       units="mm"
         )
...
ggsave(plotb, 
       filename = here("results/pub_plotD.pdf"),
       width = 180, 
       height=120,
       units="mm"
         )
